...
Vec receivedRadiance(const Ray &r, int depth, bool flag) {
    ...
    // compute intersection point, outgoing direction and nornal direction
    ...
    // Other useful quantities/variables:
    Vec Le = obj.e;                             // Emitted radiance
    const BRDF &brdf = obj.brdf;                // Surface BRDF at x

    // Call brdf.sample() to sample an incoming direction and continue the recursion
    Vec i;
    double pdf;
    brdf.sample(n, o, i, pdf);
    // printf("%f,%f,%f,%f\n", i.x, i.y, i.z, pdf); 

    // L = Le + receivedRadiance(r_i, depth+1, flag).mult(brdf.eval(n, o, i)) * pdf * i.dot(n);
    return Le + receivedRadiance(Ray(x,i), depth+1, flag).mult(brdf.eval(n, o, i)) * pdf * (1/p);
...
// Vec reflectedRadiance(Vec &, Vec &, Vec &, const Sphere &, int, bool, double);
...
// Vec reflectedRadiance(Vec &x, Vec &n, Vec &o, const Sphere &obj, int depth, bool flag, double p) {
...
//     return receivedRadiance(Ray(x,i), depth+1, flag).mult(brdf.eval(n, o, i)) * pdf * (1/p);
...
//     // return emittedRadiance(obj) + reflectedRadiance(x, o, n, obj, depth, flag, p);
//     return obj.e + reflectedRadiance(x, o, n, obj, depth, flag, p);
